...
import {
  ...
  createScrapeTarget,
  deleteScrapeTarget,
  disableScrapeTargets,
  exportScrapeRegistry,
  exportScrapeRegistryRegions,
  getRegistryRefreshStatus,
  importScrapeRegistry,
  importScrapeRegistryRegions,
  listRegistrySources,
  listScrapePlatforms,
  listScrapeRegistryOverview,
  listScrapeTargets,
  refreshRegistrySource,
  softDeleteScrapeTargets,
...
const EMPTY_SOURCE = {
  name: "NJ-CRC permitted businesses",
  state: "new-jersey",
  url: "https://www.nj.gov/cannabis/businesses/permitted/index.shtml",
  parser_key: "nj_crc_permitted_businesses",
  is_active: true,
};
...
export default function ScrapingRegistryPage() {
  ...
  const load = async () => {
    try {
      const [nextOverview, nextSources, nextPlatforms, nextTargets] =
        await Promise.all([
          listScrapeRegistryOverview(),
          listRegistrySources(),
          listScrapePlatforms(),
          listScrapeTargets(state),
        ]);
      ...
      setSelectedTargetIds((current) =>
        current.filter((id) => nextTargets.some((row) => row.id === id)),
      );
      setError("");
    } catch (requestError) {
  ...
  const sourceParserMessage =
    source?.parser_message || "No parser configured for this source.";
  useEffect(() => {
    setSourceUrl(source?.url || "");
  }, [source?.id, source?.url]);

  const supportedPlatforms = useMemo(
    () => new Set(platforms.map((item) => item.key)),
    [platforms],
  );
  const filtered = useMemo(
    () =>
      targets.filter(
        (row) =>
          `${row.name} ${row.city || ""} ${row.region || ""} ${row.website || ""} ${row.platform || ""}`
            .toLowerCase()
            .includes(query.toLowerCase()) &&
          (!platformFilter ||
            (row.platform || "Not configured") === platformFilter) &&
          (!marketFilter ||
            (marketFilter === "none"
              ? !row.market_type
  ...
  const additionalScraperKeys = Object.keys(settings).filter(
    (key) =>
      !platformSettingKeys.includes(key) && !INTERNAL_SETTINGS_KEYS.has(key),
  );
  ...
  const saveTarget = async (event) => {
    ...
    try {
      const roster_rank =
        target.roster_rank === "" ||
        target.roster_rank === null ||
        target.roster_rank === undefined
          ? null
          : Number(target.roster_rank);
  ...
  const refresh = async () => {
    try {
      ...
      const row = source
        ...
        : await createRegistrySource({
            ...EMPTY_SOURCE,
            state,
            url: sourceUrl,
          });
      if (!row.parser_configured)
        throw new Error(
          row.parser_message || "No parser configured for this source.",
        );
      ...
      if (state === "new-jersey" && result.task_id) {
        ...
        if (status.state === "error")
          throw new Error(
            status.error || "New Jersey registry refresh failed.",
          );
  ...
  const exportTargets = async () => {
    try {
      const payload = await exportScrapeRegistry(
        state,
  ...
  const importTargets = async (event) => {
    ...
    try {
      ...
      if (payload.state !== state || !Array.isArray(payload.targets))
        throw new Error("Choose a registry export for the selected state.");
      const result = await importScrapeRegistry({
        state,
        targets: payload.targets,
      });
      setNotice(
        `Registry imported: ${result.created} added, ${result.updated} updated.`,
      );
      setError("");
      load();
    } catch (requestError) {
      setError(requestError.message || "Registry import failed.");
    }
  };
  ...
  const exportRegions = async () => {
    try {
      ...
      const link = document.createElement("a");
      link.href = href;
      link.download = filename || `${state}-regions.xlsx`;
      link.click();
      URL.revokeObjectURL(href);
      setNotice("Region export downloaded.");
      setError("");
    } catch (requestError) {
      setError(requestError.message);
    }
  ...
  const importRegions = async (event) => {
    ...
    try {
      ...
      setNotice(
        `Regions imported: ${result.updated} updated, ${result.skipped} skipped.`,
      );
      setError("");
      load();
    } catch (requestError) {
      setError(requestError.message || "Region import failed.");
    } finally {
      setRegionsImporting(false);
    }
  };
  ...
  const allVisibleSelected =
    filtered.length > 0 &&
    filtered.every((row) => selectedTargetIds.includes(row.id));
  ...
  return (
    <div className="registry-page">
      <header className="registry-page__header">
        ...
        {activeState && (
          <div className="registry-page__count">
      ...
      {notice && (
        <div className="registry-alert registry-alert--success">{notice}</div>
      )}
      {error && (
        <div className="registry-alert registry-alert--error">{error}</div>
      )}
      <section className="registry-state-bar" aria-label="State selector">
        <span className="registry-state-bar__label">State</span>
        <div className="registry-state-bar__controls">
          {overview.map((item) => (
            <button
              key={item.state}
            ...
            >
              ...
              <span>
                {item.configured_active}/{item.total}
              </span>
            </button>
          ))}
        </div>
      </section>
      <section className="registry-source">
        ...
        <div className="registry-source__controls">
          <input
            className="registry-input"
            aria-label="Registry source URL"
            placeholder="https://..."
            value={sourceUrl}
        ...
        {refreshing && refreshProgress && (
          <div className="registry-alert registry-alert--success">
            {refreshProgress.message ||
              "Starting New Jersey registry refresh..."}
            {refreshProgress.percent !== undefined
              ? ` (${refreshProgress.percent}%)`
              : ""}
          </div>
        )}
        {!source?.parser_configured && (
          <div className="registry-alert registry-alert--error">
            {sourceParserMessage}
          </div>
        )}
        {source?.last_refresh_error && (
          <div className="registry-alert registry-alert--error">
            Source needs review: {source.last_refresh_error}
          </div>
        )}
      </section>
      <section className="registry-actions">
        <div className="registry-section-heading">
          <div>
            <h2>Actions</h2>
            <p>
              Export or import registry data, maintain regions, and add stores
              for the selected state.
            </p>
          </div>
        </div>
        <div className="registry-actions__controls">
          <button
          ...
          <button
            className="registry-button"
            onClick={() => importRegionsInputRef.current?.click()}
            disabled={regionsImporting}
          >
            {regionsImporting ? "Importing..." : "Import regions"}
          </button>
          <input
            ref={importRegionsInputRef}
            className="registry-file-input"
            type="file"
            accept=".xlsx"
            onChange={importRegions}
          />
          <button
            className="registry-button registry-button--primary"
            onClick={() => openTarget({ ...EMPTY_TARGET, state })}
          >
            Add store
          </button>
        </div>
      ...
      <section className="registry-table-panel">
        <div className="registry-table-panel__toolbar">
          <div>
            ...
            <p>
              {filtered.length} of {targets.length} records
            </p>
          </div>
          <div className="registry-table-panel__actions">
            <select
            ...
            >
              ...
              {SCRAPER_STATUS_OPTIONS.map(([value, label]) => (
            ...
            <input
              className="registry-input registry-input--search"
            ...
            {selectedTargetIds.length > 0 && (
              <>
                <button className="registry-button" onClick={disableSelected}>
                  Disable selected ({selectedTargetIds.length})
                </button>
                <button
            ...
            <thead>
              <tr>
                <th>
                  <input
                    type="checkbox"
                    aria-label="Select visible stores"
                    checked={allVisibleSelected}
                    onChange={toggleVisibleTargets}
                  />
                </th>
                {[
                  "Store",
                ...
                <tr key={row.id}>
                  <td>
                    <input
                      ...
                      onChange={() => toggleTarget(row.id)}
                    />
                  </td>
                  <td className="registry-table__store">{row.name}</td>
                  <td>{row.city || "—"}</td>
                  <td>{row.region || "—"}</td>
                  <td>{row.market_type ? stateLabel(row.market_type) : "—"}</td>
                  <td>
                    {row.website ? (
                  ...
                  <td>
                    <div className="registry-row-actions">
                      <button
                        className="registry-button registry-button--quiet"
                        onClick={() => openTarget(row)}
                      >
                        Edit
                      </button>
                      <button
                        className="registry-button registry-button--quiet registry-button--danger"
        ...
        {!filtered.length && (
          <div className="registry-table-empty">
      ...
      {target && (
        <div className="registry-editor-backdrop">
          <form onSubmit={saveTarget} className="registry-editor">
            ...
            <div className="registry-fields">
              {[
                ...
                <label key={key}>
                  ...
                  <input
                    ...
                    value={target[key] || ""}
                    onChange={(event) =>
                      setTarget({ ...target, [key]: event.target.value })
                    }
                  />
                </label>
              ))}
              <label>
                ...
                <select
                  className="registry-input"
                  value={target.market_type || ""}
                  onChange={(event) =>
                    setTarget({ ...target, market_type: event.target.value })
                  }
                >
            ...
            <section className="registry-settings">
              <div className="registry-settings__heading">
                <div>
                  <h3>OOS ranking roster</h3>
                  <p>
                    Set roster status to put this store on the NJ ranking
                    roster; leave it blank to keep it out of ranking, pricing,
                    and insights.
                  </p>
                </div>
              </div>
              <div className="registry-fields" style={{ marginTop: 14 }}>
                {[
                  ["Tier", "tier"],
                  ["Roster status", "roster_status"],
                  ["Reviewed", "roster_reviewed_at"],
                ].map(([label, key]) => (
                  <label key={key}>
                    {label}
                    <input
                ...
                <label>
                  ...
                  <input
                    type="number"
                    className="registry-input"
                    value={target.roster_rank ?? ""}
                    onChange={(event) =>
                      setTarget({ ...target, roster_rank: event.target.value })
                    }
                  />
                </label>
                <label className="registry-field--wide">
                  Roster notes
                  <textarea
                    className="registry-input registry-input--textarea"
                    value={target.roster_notes || ""}
                    onChange={(event) =>
                      setTarget({ ...target, roster_notes: event.target.value })
                    }
                  />
                </label>
              </div>
            ...
            <section className="registry-settings">
              ...
              {platformConfig ? (
                <div className="registry-settings__grid">
                  {platformSettingKeys.map((key) => (
                    <SettingField
                      key={key}
                      settingKey={key}
                      value={settings[key]}
                      required={key === "store_id"}
                      onChange={updateSetting}
                    />
                  ))}
                  {additionalScraperKeys.map((key) => (
                    <SettingField
                      key={key}
                      settingKey={key}
                      value={settings[key]}
      ...
      {deleting && (
        <ConfirmModal
          ...
          message="This disables the store and removes it from the active OOS registry. Its record is retained for audit purposes."
      ...
      {bulkDeleting && (
        <ConfirmModal
          title={`Delete ${selectedTargetIds.length} stores?`}
          message="This disables the selected stores and removes them from the active OOS registry. Their records are retained for audit purposes."
...
function SettingField({ settingKey, value, required, onChange }) {
  ...
  return (
    <label
      className={`registry-setting-field${required ? " is-required" : ""}`}
    >
      {fieldLabel(settingKey)}
      {isBoolean ? (
        <span className="registry-setting-toggle">
          <input
            type="checkbox"
            checked={value}
      ...
      {required && (
        <span className="registry-setting-field__required">Required</span>
      )}
    </label>
  );
}
