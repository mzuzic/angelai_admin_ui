...
export default function ScrapingRegistryPage() {
  ...
  const load = async () => {
    try {
      const [nextOverview, nextSources, nextPlatforms, nextTargets] = await Promise.all([
        listScrapeRegistryOverview(), listRegistrySources(), listScrapePlatforms(), listScrapeTargets(state),
      ])
      setOverview(nextOverview); setSources(nextSources); setPlatforms(nextPlatforms); setTargets(nextTargets); setSelectedTargetIds((current) => current.filter((id) => nextTargets.some((row) => row.id === id))); setError('')
  ...
  const saveTarget = async (event) => {
    ...
    try {
      ...
      setTarget(null); setNotice('Scrape target saved.'); setError(''); load()
    } catch (requestError) { setError(requestError.message) }
  }

  const refresh = async () => {
    try {
      setRefreshing(true); setRefreshProgress(null); setError('')
      const row = source
      ...
      let result = await refreshRegistrySource(row.id)
      if (state === 'new-jersey' && result.task_id) {
        let status = await getRegistryRefreshStatus(result.task_id)
        while (status.state === 'running') {
          setRefreshProgress(status.progress || {})
          await new Promise((resolve) => setTimeout(resolve, 1000))
          status = await getRegistryRefreshStatus(result.task_id)
        }
        if (status.state === 'error') throw new Error(status.error || 'New Jersey registry refresh failed.')
      ...
      load()
    } catch (requestError) { setError(requestError.message) } finally { setRefreshing(false); setRefreshProgress(null) }
  }

  const deleteTarget = async () => {
    try {
      await deleteScrapeTarget(deleting.id)
      setDeleting(null); setTarget(null); setNotice('Scrape target deleted and disabled.'); setError(''); load()
    } catch (requestError) { setError(requestError.message) }
  }
  ...
  const disableSelected = async () => {
    try {
      ...
      setSelectedTargetIds([]); setNotice(`${result.disabled} scrape target${result.disabled === 1 ? '' : 's'} disabled.`); setError(''); load()
    } catch (requestError) { setError(requestError.message) }
  }
  const deleteSelected = async () => {
    try {
      const result = await softDeleteScrapeTargets(selectedTargetIds)
      setBulkDeleting(false); setSelectedTargetIds([]); setNotice(`${result.deleted} scrape target${result.deleted === 1 ? '' : 's'} deleted and disabled.`); setError(''); load()
    } catch (requestError) { setError(requestError.message) }
  }
