...
const SCRAPER_STATUS_OPTIONS = [
  ['new', 'New'], ['delisted', 'Delisted'], ['missing', 'No website'], ['undetected', 'No platform'], ['unsupported', 'No scraper'],
...
function formatDate(value) {
  ...
  return Number.isNaN(date.getTime()) ? 'Not scraped' : date.toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })
}
...
function scraperStatus(row, supportedPlatforms) {
  ...
  if (row.platform) return { key: 'unsupported', label: 'No scraper' }
...
export default function ScrapingRegistryPage() {
  ...
  const sourceParserMessage = source?.parser_message || 'No parser configured for this source.'
  ...
  const filtered = useMemo(() => targets.filter((row) => (
    `${row.name} ${row.city || ''} ${row.website || ''} ${row.platform || ''}`.toLowerCase().includes(query.toLowerCase())
  ...
  const saveTarget = async (event) => {
    ...
    try {
      ...
      setTarget(null); setNotice('Scrape target saved.'); load()
    } catch (requestError) { setError(requestError.message) }
  }
  ...
  const deleteTarget = async () => {
    try {
      ...
      setDeleting(null); setTarget(null); setNotice('Scrape target deleted and disabled.'); load()
    } catch (requestError) { setError(requestError.message) }
  }
  ...
  const disableSelected = async () => {
    try {
      ...
      setSelectedTargetIds([]); setNotice(`${result.disabled} scrape target${result.disabled === 1 ? '' : 's'} disabled.`); load()
    } catch (requestError) { setError(requestError.message) }
  }
  const deleteSelected = async () => {
    try {
      const result = await softDeleteScrapeTargets(selectedTargetIds)
      setBulkDeleting(false); setSelectedTargetIds([]); setNotice(`${result.deleted} scrape target${result.deleted === 1 ? '' : 's'} deleted and disabled.`); load()
    } catch (requestError) { setError(requestError.message) }
  }
  ...
  const importTargets = async (event) => {
    ...
    try {
      ...
      setNotice(`Registry imported: ${result.created} added, ${result.updated} updated.`)
      load()
  ...
  return <div className="registry-page">
    <header className="registry-page__header"><div><div className="registry-page__eyebrow">OOS Operations</div><h1>Scraping registry</h1><p>Manage stores, scrape configuration, and the public registry source for each state.</p></div>{activeState && <div className="registry-page__count"><strong>{activeState.total}</strong><span>registered stores</span></div>}</header>
    ...
    <section className="registry-source"><div className="registry-section-heading"><div><h2>Registry source</h2><p>Refresh updates the official retailer list and links verified scraper targets.</p></div></div><div className="registry-source__controls"><input className="registry-input" aria-label="Registry source URL" placeholder="https://..." value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} /><button className="registry-button registry-button--primary" onClick={refresh} disabled={!source || !sourceUrl.trim() || !source.parser_configured || refreshing}>{refreshing ? 'Refreshing...' : 'Refresh registry'}</button></div>{!source?.parser_configured && <div className="registry-alert registry-alert--error">{sourceParserMessage}</div>}{source?.last_refresh_error && <div className="registry-alert registry-alert--error">Source needs review: {source.last_refresh_error}</div>}</section>
    {publicRecords.length > 0 && <section className="registry-table-panel"><div className="registry-table-panel__toolbar"><div><h2>Public registry</h2><p>{publicRecords.length} official records; {publicMatched} linked to scraper targets</p></div></div><div className="registry-table-scroll"><table className="registry-table"><thead><tr>{['Legal name', 'City', 'Address', 'Registry ID', 'Type', 'Scraper target'].map((heading) => <th key={heading}>{heading}</th>)}</tr></thead><tbody>{publicRecords.map((row) => <tr key={row.id}><td className="registry-table__store">{row.legal_name}</td><td>{row.city || '—'}</td><td>{row.address || '—'}</td><td>{row.registry_id}</td><td>{row.business_type || '—'}</td><td>{row.scrape_target_id ? 'Matched' : 'Needs review'}</td></tr>)}</tbody></table></div></section>}
    <section className="registry-table-panel"><div className="registry-table-panel__toolbar"><div><h2>{stateLabel(state)} stores</h2><p>{filtered.length} of {targets.length} records</p></div><div className="registry-table-panel__actions"><select className="registry-input registry-filter" aria-label="Filter by platform" value={platformFilter} onChange={(event) => setPlatformFilter(event.target.value)}><option value="">All platforms</option><option value="Not configured">Not configured</option>{platforms.map((platform) => <option key={platform.key} value={platform.key}>{platform.key}</option>)}</select><select className="registry-input registry-filter" aria-label="Filter by status" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">All statuses</option>{SCRAPER_STATUS_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><input className="registry-input registry-input--search" placeholder="Search stores..." value={query} onChange={(event) => setQuery(event.target.value)} />{selectedTargetIds.length > 0 && <><button className="registry-button" onClick={disableSelected}>Disable selected ({selectedTargetIds.length})</button><button className="registry-button registry-button--danger" onClick={() => setBulkDeleting(true)}>Delete selected ({selectedTargetIds.length})</button></>}<button className="registry-button" onClick={exportTargets} disabled={!filtered.length}>Export for agent</button><button className="registry-button" onClick={() => importInputRef.current?.click()}>Import changes</button><input ref={importInputRef} className="registry-file-input" type="file" accept="application/json" onChange={importTargets} /><button className="registry-button registry-button--primary" onClick={() => openTarget({ ...EMPTY_TARGET, state })}>Add store</button></div></div><div className="registry-table-scroll"><table className="registry-table"><colgroup><col style={{ width: '36px' }} /><col style={{ width: '21%' }} /><col style={{ width: '13%' }} /><col style={{ width: '9%' }} /><col style={{ width: '13%' }} /><col style={{ width: '11%' }} /><col style={{ width: '17%' }} /><col style={{ width: '15%' }} /></colgroup><thead><tr><th><input type="checkbox" aria-label="Select visible stores" checked={allVisibleSelected} onChange={toggleVisibleTargets} /></th>{['Store', 'City', 'Website', 'Platform', 'Status', 'Last scraped', ''].map((heading) => <th key={heading}>{heading}</th>)}</tr></thead><tbody>{filtered.map((row) => <tr key={row.id}><td><input type="checkbox" aria-label={`Select ${row.name}`} checked={selectedTargetIds.includes(row.id)} onChange={() => toggleTarget(row.id)} /></td><td className="registry-table__store">{row.name}</td><td>{row.city || '—'}</td><td>{row.website ? <a href={row.website} target="_blank" rel="noreferrer">Open site</a> : '—'}</td><td>{row.platform || '—'}</td><StatusCell row={row} supportedPlatforms={supportedPlatforms} /><td>{formatDate(row.last_scraped_at)}</td><td><div className="registry-row-actions"><button className="registry-button registry-button--quiet" onClick={() => openTarget(row)}>Edit</button><button className="registry-button registry-button--quiet registry-button--danger" onClick={() => setDeleting(row)}>Delete</button></div></td></tr>)}</tbody></table></div>{!filtered.length && <div className="registry-table-empty">No stores match this search.</div>}</section>
    {target && <div className="registry-editor-backdrop"><form onSubmit={saveTarget} className="registry-editor"><div className="registry-editor__heading"><div><h2>{target.id ? 'Edit scrape target' : 'Add scrape target'}</h2><p>Store details are saved to the OOS registry. Platform settings control scraping.</p></div><button type="button" className="registry-close" onClick={() => setTarget(null)} aria-label="Close editor">×</button></div><div className="registry-fields">{[['Name', 'name'], ['City', 'city'], ['Address', 'address'], ['License', 'license'], ['Website', 'website']].map(([label, key]) => <label key={key}>{label}<input className="registry-input" value={target[key] || ''} onChange={(event) => setTarget({ ...target, [key]: event.target.value })} /></label>)}<label>Platform<select className="registry-input" value={target.platform || ''} onChange={(event) => setTarget({ ...target, platform: event.target.value })}><option value="">Not configured</option>{isLegacyPlatform && <option value={target.platform}>{target.platform} (not supported)</option>}{platforms.map((platform) => <option key={platform.key} value={platform.key}>{platform.key}</option>)}</select></label></div><section className="registry-settings"><div className="registry-settings__heading"><div><h3>Scraper settings</h3><p>{platformConfig ? settings.store_id ? 'This store is configured for the selected scraper.' : 'A scraper is available. Add the Store ID to configure this store.' : 'Fields vary by the selected platform. Store ID is required; remaining fields are optional overrides.'}</p></div></div>{platformConfig ? <div className="registry-settings__grid">{platformSettingKeys.map((key) => <SettingField key={key} settingKey={key} value={settings[key]} required={key === 'store_id'} onChange={updateSetting} />)}{additionalScraperKeys.map((key) => <SettingField key={key} settingKey={key} value={settings[key]} onChange={updateSetting} />)}</div> : <p className="registry-settings__empty">{isLegacyPlatform ? 'No scraper is available for this platform. Choose a supported platform after verification.' : target.website ? 'A website is recorded, but no platform has been identified.' : 'No website is recorded, so a platform cannot be verified.'}</p>}</section><label className="registry-toggle"><input type="checkbox" checked={target.is_active} onChange={(event) => setTarget({ ...target, is_active: event.target.checked })} />Active scrape target</label><div className="registry-editor__actions"><button className="registry-button registry-button--primary" type="submit">Save target</button>{target.id && <button className="registry-button registry-button--danger" type="button" onClick={() => setDeleting(target)}>Delete</button>}<button className="registry-button" type="button" onClick={() => setTarget(null)}>Cancel</button></div></form></div>}
